...
class ConfigParser:
    ...
    def parse(self, path: str | Path) -> EntityConfig:
        ...
        logger.info(
            "Config parsed successfully: entity=%s, sources=%d, joins=%d, transforms=%d, validations=%d, fields=%d",
            config.entity.name,
            len(config.sources),
            len(config.joins or []),
            len(config.transforms or []),
    ...
    def _replace_env_vars(value: str) -> str:
        """Replace all ``${VAR}`` patterns in a string with environment values."""
        ...
        def _replacer(match: re.Match) -> str:
            ...
            if default is not None:
                return default
            # Return the original placeholder — Pydantic will catch it if required
